...
public:
    // We need to fill array with zeros, so no thrash in memory will change results of calculations
    AirportCounter() {
        storage.fill(0);
    }
    // Logic of this program is very simple - simple operations on very fast container
    template <typename TIterator>
    AirportCounter(const TIterator& begin, const TIterator& end) {
        storage.fill(0);
        for (auto it = begin; it != end; it++)
            storage[static_cast<uint32_t>(*it)]++;
    }

    [[nodiscard]] size_t Get(const TAirport& airport) const {
        return storage[static_cast<uint32_t>(airport)];
    }

    void Insert(const TAirport& airport) {
        storage[static_cast<uint32_t>(airport)]++;
    }
    ...
    void EraseAll(const TAirport& airport) {
        storage[static_cast<uint32_t>(airport)] = 0;
    }
    ...
    [[nodiscard]] Items GetItems() const {
        Items items;
        // Just create array linearly, it won't take very long
        for (size_t i = 0; i < storage.size(); ++i) {
            items[i] = {static_cast<TAirport>(i), storage[i]};
        }

        return items;
    }
// We know size, so we'll use array
private:
    array<size_t, static_cast<uint32_t>(TAirport::Last_)> storage;
};
// Full test suite, provided by authors
void TestMoscow() {
